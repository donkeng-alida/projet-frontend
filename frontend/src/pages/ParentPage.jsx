...
function ParentPage({ theme, langue, onToggleTheme, onLangueChange }) {
  ...
  const textes = {
    fr: {
      titre: 'Espace Parent',
      sousTitre: 'Bulletin de votre enfant',
      notesRecues: 'Notes publiees',
      notesIntro: 'Vous voyez uniquement les notes de votre enfant.',
      notesVides: 'Aucune note publiee pour le moment.',
    ...
    en: {
      titre: 'Parent Space',
      sousTitre: 'Your child report card',
      notesRecues: 'Published marks',
      notesIntro: "You can only view your child's marks.",
      notesVides: 'No published mark yet.',
      ...
      accueil: 'Home',
      connexionTitre: 'Parent Login',
      identifiant: 'Username (email)',
      seConnecter: 'Sign in',
      deconnexion: 'Sign out',
      motDePasseLogin: 'Password',
      accesRefuse: 'Access denied: this account is not parent.',
  ...
  const isParent = authToken && (authScope === 'superuser' || authRole === 'parent');
  ...
  const handleLogin = async () => {
    ...
    try {
      ...
      if (data?.role !== 'parent' && !data?.is_superuser) {
      ...
      const nextScope = data?.is_superuser ? 'superuser' : 'parent';
